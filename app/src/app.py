...
#cursor.execute("CREATE TABLE IF NOT EXISTS `forms_data`.`formdata_table` ( `user_id` BIGINT UNIQUE AUTO_INCREMENT, `name` VARCHAR(40) NULL, `tno` VARCHAR(5) NULL, `size` VARCHAR(5) NULL, PRIMARY KEY (`user_id`) ); ")
#conn.commit()
...
def form():
	...
	cursor.execute("insert into formdata_table (name, tno, size) values (%s,%s,%s)",(_name, _tno, _size))
